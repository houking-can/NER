    ...
    with open(file_path, encoding="utf-8") as f:
        for line in f:
            ...
            for span in spans:
                ...
                labels[start] = "B-%s" % span["label"]
                for i in range(start + 1, end):
                    labels[i] = "I-%s" % span["label"]
            examples.append((words, labels))
    return examples


def json2txt(data_dir):
    for name in ['train', 'dev', 'test']:
        examples = read_examples_from_json(data_dir, name)
        with open('./%s/%s.txt' % (data_dir,name), 'w', encoding='utf-8') as f:
            for each in examples:
                for word, label in zip(each[0], each[1]):
                    word = word.lower()
                    f.write('%s %s\n' % (word, label))
                f.write('\n')
